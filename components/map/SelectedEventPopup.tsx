        ...
        <Fragment key={`trajectory-${selectedEvent.id}-${styleVersion}`}>
          ...
          {selectedEvent.geometry
            ...
            .map((g, i) => {
              ...
              const mag = g.magnitudeValue
                ? `${g.magnitudeValue} ${g.magnitudeUnit || ""}`
                : null;

              return (
                <MapMarker
                  key={`${selectedEvent.id}-wp-${i}`}
                  longitude={g.coordinates[0]}
                  latitude={g.coordinates[1]}
                >
                  <MarkerContent>
                    <div
                      className="h-2.5 w-2.5 rounded-full border border-white/60 transition-transform hover:scale-150"
                  ...
                  <MarkerTooltip className="text-xs">
                    ...
                    <div className="text-muted-foreground text-[10px]">
                      {g.coordinates[1].toFixed(2)},{" "}
                      {g.coordinates[0].toFixed(2)}
                    </div>
                  </MarkerTooltip>
                </MapMarker>
              );
        ...
        <MarkerContent>
          ...
          >
            <span className="text-base leading-none">
              {getCategoryEmoji(selectedEvent)}
            </span>
          </div>
        </MarkerContent>
      </MapMarker>
          ...
          {illustration && (
            <div className="relative h-24 -mb-3">
              ...
              <div className="absolute top-2 left-2 flex gap-1">
                ...
                >
                  <CategoryIcon
                    categoryId={selectedEvent.categories[0]?.id || ""}
                    className="h-2.5 w-2.5 mr-0.5"
                  />
                  {selectedEvent.categories[0]?.title}
                </Badge>
                <Badge
                  className={`text-[9px] h-5 backdrop-blur-sm ${isOpen ? "bg-green-600/90 text-white" : "bg-muted/80 text-muted-foreground"}`}
                >
          ...
          <CardContent className="space-y-2.5">
            {!illustration && (
              <div className="flex flex-wrap gap-1">
                ...
                <Badge
                  className={`text-[10px] h-5 ${isOpen ? "bg-green-500/10 text-green-500 border-green-500/20" : ""}`}
                  variant={isOpen ? undefined : "secondary"}
                >
                  {isOpen ? "Active" : "Closed"}
                </Badge>
              </div>
