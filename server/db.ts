...
const cache = new Map<string, { data: unknown; generation: number; ts: number }>()
...
async function readWithGeneration<T>(path: string, fallback: T): Promise<{ data: T; generation: number }> {
  const cached = cache.get(path)
