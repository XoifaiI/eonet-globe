...
async function uploadBuffer(gcsPath: string, buffer: Buffer, contentType: string): Promise<number> {
...
export async function promoteFromQuarantine(filename: string): Promise<void> {
  const variants = ["original.webp.zst", "medium.webp.zst", "thumbnail.webp.zst"]
  await Promise.all(
    variants.map((v) =>
      bucket.file(`quarantine/${filename}/${v}`).move(`images/${filename}/${v}`)
    )
  )
}
