...
export default memo(function StatsCard() {
  ...
  const stats = useMemo(() => {
    ...
    const cutoff = Date.now() - MS_PER_WEEK; // eslint-disable-line react-hooks/purity
    let recentCount = 0;
    let topCategory = "";
    let topCategoryId = "";
    let topCount = 0;
    const catCounts = new globalThis.Map<
      string,
      { title: string; id: string; count: number }
    >();

    for (const e of events) {
      const geo = e.geometry[e.geometry.length - 1];
      if (geo && new Date(geo.date).getTime() > cutoff) recentCount++;
  ...
  const topColor =
    CATEGORY_COLORS[stats.topCategoryId] || DEFAULT_CATEGORY_COLOR;
      ...
      <CardContent className="space-y-3">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs">
            ...
            >
              <CategoryIcon
                categoryId={stats.topCategoryId}
