...
import { formatEventDate, CATEGORY_COLORS } from "@/lib/eonet";
...
const SidebarItem = memo(function SidebarItem({
  ...
  const color = CATEGORY_COLORS[categoryId] || DEFAULT_CATEGORY_COLOR;
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1 text-left">
        ...
        <span
          className={`text-[10px] ${isSelected ? "text-primary-foreground/60" : "text-muted-foreground"}`}
        >
  ...
  const handleSelect = useCallback(
    (eventId: string) => {
      startTransition(() => {
        ...
        const next =
          current === eventId ? null : (state.eventsById.get(eventId) ?? null);
            ...
            <DrawerHeader className="pb-0">
              <div className="flex items-center gap-2">
                ...
                {!loading && (
                  <Badge
                    variant="outline"
                    className="text-[10px] h-5 font-mono tabular-nums ml-auto"
                  >
                    {events.length}
                  </Badge>
                )}
